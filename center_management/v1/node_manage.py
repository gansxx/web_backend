...
try:
	...
	# fallback: load by file path from the parent directory (since we're in v1/ subdirectory)
	spec_path = Path(__file__).resolve().parent.parent / 'vps_vultur_manage.py'
...
def run_remote_self_sb_change(hostname, port, username, key_file, port_arg=None, name_arg=None, up_mbps=None, down_mbps=None, script_path='/root/sing-box-v2ray/self_sb_change.sh', proxy=None):
	...
	参数:
		...
		down_mbps: 下载带宽限制
		script_path: 脚本路径
		proxy: 可选的NodeProxy对象，用于复用SSH连接
	...
	# 使用提供的proxy或创建新的
	if proxy is None:
		with NodeProxy(hostname, port, username, key_file) as proxy_to_use:
			exit_status, out, err = proxy_to_use.execute_command(command)
	else:
...
def find_database_file(hostname, username, key_file, possible_paths=None, timeout=10, proxy=None):
	"""在远程服务器上查找数据库文件
	...
	# 使用提供的proxy或创建新的
	if proxy is None:
		with NodeProxy(hostname, 22, username, key_file, timeout=timeout) as proxy_to_use:
			for path in possible_paths:
				# 检查文件是否存在
				cmd = f"test -f '{path}' && echo 'exists' || echo 'not_exists'"
				exit_status, out, err = proxy_to_use.execute_command(cmd, timeout=10)
...
def fetch_db_data_direct(hostname, username, key_file, table_names=None, db_path=None, timeout=10, proxy=None):
	...
	# 如果没有指定数据库路径，自动查找
	if db_path is None:
		db_path = find_database_file(hostname, username, key_file, timeout=timeout, proxy=proxy)
		if not db_path:
...
def get_remote_ports_by_protocol(hostname, port=22, username='root', key_file=None, timeout=10, proxy=None):
	...
	try:
		...
		cmd = "ss -tuln"
		exit_status, output, error = proxy_to_use.execute_command(cmd, timeout=30)
